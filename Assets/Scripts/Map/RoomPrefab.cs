...
namespace RogueGame.Map
{
    ...
    public sealed class RoomPrefab : MonoBehaviour
    {
        ...
        {
            CDTU.Utils.CDLogger.Log($"[RoomPrefab] OpenAllDoors 调用，门数量: {Doors.Count}");
        ...
        {
            CDTU.Utils.CDLogger.Log($"[RoomPrefab] CloseAllDoors 调用，门数量: {Doors.Count}");
            foreach (var door in Doors)
            {
                if (door != null)
                {
        ...
        {
            CDTU.Utils.CDLogger.Log($"[RoomPrefab] LockAllDoors 调用，门数量: {Doors.Count}");
            foreach (var door in Doors)
            {
                if (door != null)
                {
        ...
        {
            CDTU.Utils.CDLogger.Log("[RoomPrefab] FullReset 调用（已移除访问标记逻辑，保留门重置）");
            foreach (var door in Doors)
            {
                door?.Reset();
            }
