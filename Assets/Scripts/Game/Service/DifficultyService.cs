...
public sealed class DifficultyService
{
    ...
    public void ApplyDifficultyScaling(GameObject enemy, int floor, object source)
    {
        ...
        if (stats == null)
        {
            Debug.LogWarning($"[DifficultyService] 敌人 {enemy.name} 缺少 CharacterStats 组件，跳过难度缩放");
            return;
        }
        ...
        {
            var speedMod = new StatModifier(atkSpeedMultiplier - 1f, StatModType.PercentAdd, source);
        ...
        Debug.Log($"[DifficultyService] 已应用 {floor} 层难度缩放到 {enemy.name}：" +
