...
//                     // slot.Energy is stored in skill-specific units; try to normalize by loading SkillData
//                     float max = 100f;
//                     var id = slot.EquippedSkillId;
//                     if (!string.IsNullOrEmpty(id))
//                     {
//                         var data = Character.Skills.SkillDatabase.GetSkillData(id);
//                         if (data != null) max = Math.Max(1f, data.maxEnergy);
//                     }
//                     _view.SetSkillSlotEnergy(i, max > 0f ? slot.Energy / max : 0f);
